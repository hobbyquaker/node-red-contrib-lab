...
module.exports = class Hm310 extends EventEmitter {
    constructor(options) {
        ...
        this.client._port.on('open', (e) => {
            console.log('hm310 open', e)
        })

        this.client._port.on('close', e => {
            console.log('hm310 close', e)
        })
    ...
    open() {
        if (!this.client || !this.client.isOpen || !this.client.isOpen()) {
            return this.client.open(error => {
                if (error) {
                    this.emit('error', error);
                    setTimeout(() => this.open(), 10000);
                } else if (!this.connected) {
    ...
    write(key, value) {
        ...
        switch (key) {
            case 'powerSwitch':
                this.enqueue(() => {
                    return this.client.writeRegisters(0x01, [value ? 1 : 0]);
                });
                break;

            case 'protectVoltage':
                this.enqueue(() => this.client.writeRegisters(0x20, [Math.round(value * 100)]));
                break;

            case 'protectCurrent':
                this.enqueue(() => this.client.writeRegisters(0x21, [Math.round(value * 1000)]));
                break;
            ...
            case 'setVoltage':
                this.enqueue(() => {
                    //console.log('write setVoltage', Math.round(value * 100));
                    return this.client.writeRegisters(0x30, [Math.round(value * 100)]);
                });
                break;

            case 'setCurrent':
                this.enqueue(() => {
                    //console.log('write setCurrent', Math.round(value * 1000));
                    return this.client.writeRegisters(0x31, [Math.round(value * 1000)]);
                });
                break;
            ...
            default: {
                ...
                if (match && match[2] === 'Voltage') {
                    ...
                    return this.client.writeRegisters(register, [Math.round(value * 100)]);
                }

                if (match && match[2] === 'Current') {
                    const register = 0x1001 + ((Number(match[1]) - 1) * 0x10);
                    return this.client.writeRegisters(register, [Math.round(value * 1000)]);
                }
            }
        }
    }
